...
public class Elude : MonoBehaviour
{
    ...
    private bool IsForwardUp = false; //前下方是否有障碍物
    ...
    void Detection()
    {
        ...
        IsForwardUp = DirectionForwardUp();
        IsInLeft = DirectionLeft();
        IsInRight = DirectionRight();

        if (IsForward||IsForwardUp)
        {
            m_transform.Translate(Vector3.up * Time.deltaTime * MoveSpeed, Space.World);
            m_transform.Translate(-Vector3.left * Time.deltaTime * MoveSpeed, Space.World);
            /*
    ...
    bool DirectionForward()
    {
        ...
        if (Physics.Raycast(m_transform.position, LocalForward, out hitForward, ColliderDistance ))
        {
    ...
    //检测前下方
    bool DirectionForwardUp()
    {
        bool check = false;
        RaycastHit hitForward;
        Vector3 LocalForward = m_transform.TransformPoint(Vector3.forward - Vector3.up) - m_transform.position;
        if (Physics.Raycast(m_transform.position, LocalForward, out hitForward, ColliderDistance))
        {
            if (CompareTags(hitForward.transform.gameObject.tag))
            {
    ...
    void OnDrawGizmos()
    {
        ...
        Gizmos.DrawLine(transform.position, transform.TransformPoint((Vector3.forward) * ColliderDistance));
        Gizmos.DrawLine(transform.position, transform.TransformPoint((Vector3.forward-Vector3.up) * ColliderDistance ));
